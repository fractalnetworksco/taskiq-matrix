...
async def test_matrix_result_backend_is_result_ready_room_message_error(
    ...
    with patch(
        ...
        with patch(
            "taskiq_matrix.matrix_result_backend.run_sync_filter", new=AsyncMock(return_value={})
...
async def test_matrix_result_backend_get_result_decode_error_fetching_result_from_matrix(
    test_matrix_result_backend,
):
    """
    ...
    with patch(
        "taskiq_matrix.matrix_result_backend.run_sync_filter", new=AsyncMock(return_value=None)
    ):
        # patch the file's logger to make sure the correct exception is raised
        with patch("taskiq_matrix.matrix_result_backend.logger", new=MagicMock()) as mock_logger:
            with pytest.raises(Exception):
